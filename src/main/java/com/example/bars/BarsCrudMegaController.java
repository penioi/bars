...
public class BarsCrudMegaController {
    ...
    public BarPrices getCurrentBarPrices(@PathVariable(name = "barId") Long barId) {
        // todo - this can be done much better
        Optional<Bar> bar = barRepo.findById(barId);
        if (bar.isPresent()) {
            Iterable<BarPrices.ProductPrice> productPrices = StreamSupport.stream(currentPriceRepo.findByBar(bar.get()).spliterator(), false).map(currentPrice -> {
                currentPrice.setBar(null);
                return currentPrice;
            }).map(cp -> new BarPrices.ProductPrice(cp.getProduct(), cp.getPrice())).
                    collect(Collectors.toList());
            BarPrices barPrices = new BarPrices(bar.get(), productPrices);
            return barPrices;
        } else {
            return null;
        }
